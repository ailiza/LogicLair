...
function destinations(teleports, dieSides, start, end) {
    const map = new Map();
    for (const teleport of teleports) {
        const pair = teleport.split(",");
        map.set(Number(pair[0]), Number(pair[1]));
    }

    const result = new Set();
    for (let idx = 1; idx <= dieSides; idx++) {
        const current = start + idx;
        if (current > end) break;
        result.add(map.get(current) || current);
    }
    return result
}